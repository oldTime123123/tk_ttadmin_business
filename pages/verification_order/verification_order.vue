<template>
	<view class="verification-list">
		...
		<block v-for="(tabItem, tabIndex) in tabsList" :key="tabItem['sign']">
			<mescroll-uni v-if="currentTab['sign'] === tabItem['sign']" top="80rpx"
				:ref="'scroll' + '-' + tabItem['sign']" @init="mescrollInit" :up="{
                    noMoreSize: 5,
                    empty: {
		...
		<view class="operation">
			<view class="operation-button" style="background-color: #ffffff; color: #000000"
			...
			<!-- 手动输入核销码MODAL -->
			<u-modal ref="uModalInput" v-model="showInputCode" show-cancel-button :confirm-text="$t('确定')"
				@confirm="inputOrderCode" :title="$t('手动核销')">
				<view class="slot-content row-center" style="padding: 40rpx">
					<u-input type="number" v-model="code" :border="true" :placeholder="$t('请输入核销码')"
						style="width: 100%" />
				</view>
			</u-modal>
		</view>
