		...
		<view class="order-contain">
			<view class="order">
				...
				<view class="order-header">
					<view class="order-sn">{{$t('联系人')}}:
						{{ orderInfo.show_verification_nickname }}({{orderInfo.show_verification_mobile}})
					</view>
			...
			<view class="operation operation--primary" @click="showVerificationModal = true">{{$t('已提货')}}</view>
			<view class="operation operation--normal" @click="goVerificationList">{{$t('返回核销列表')}}</view>
		</view>

		<!-- 二次确认核销 -->
		<u-modal ref="uModalInput" v-model="showVerificationModal" show-cancel-button :confirm-text="$t('确定')"
			:title="$t('确认核销')" :content="$t('是否确认核销？')" @confirm="handleVerificationConfirm" />

		<!-- 页面状态 -->
		<page-status :status="pageStatus">
			<view slot="error" class="flex-col column-center">
				<u-empty :text="$t('订单异常')" src="/static/images/empty/order.png" :icon-size="280"></u-empty>
				<view class="operation operation--primary" @click="goVerificationList">{{$t('返回核销列表')}}</view>
			</view>
		</page-status>
	</view>
</template>
	...
	export default {
		...
		async onLoad() {
			...
			try {
				...
				if (!options.code) throw new Error(this.$t('订单异常'));
