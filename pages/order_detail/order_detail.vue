<template>
  <view class="order-detail">
    ...
    <view class="address-wrap flex contain bg-white row-between">
      ...
      <view class="copy" @click="copy(orderInfo.delivery_address)">  {{$t('复制')}}</view>
    </view>

    <view class="m-t-20 bg-white">
      <!-- Header -->
      <view class="order-header flex row-between">
        <view class="black md bold">  {{$t('商品信息')}}</view>
      ...
      <u-collapse>
        <u-collapse-item v-for="(item, index) in itemList" :key="index">
          <view
            ...
            style="width: 100%"
          >
            <view class="normal">{{$t('支付方式')}}：{{ orderInfo.pay_way_text }}</view>
            <view class="lighter"
              >{{$t('实际付款')}}：<text style="color: #ff0000"
          ...
          <view class="flex m-b-20 row-around">
            <view>{{$t('商品金额')}}： ¥{{ orderInfo.goods_price }}</view>
            <view>{{$t('运费金额')}}： ¥{{ orderInfo.shipping_price }}</view>
          </view>
          <view class="m-b-40 flex row-around">
            <view>{{$t('会员优惠')}}： ¥{{ orderInfo.member_amount || 0 }}</view>
            <view>{{$t('优惠金额')}}： ¥{{ orderInfo.discount_amount }}</view>
          </view>
        </u-collapse-item>
      </u-collapse>
    </view>
    ...
    >
      <view class="black md bold"> {{$t('发货内容')}} </view>
      <view class="normal sm m-t-20">
        ...
        >
          <view class="copy-btn">{{$t('复制')}}</view>
        </view>
      </view>
    </view>

    <view class="bg-white m-t-20" style="padding: 20rpx 20rpx 50rpx 20rpx">
      <view class="black md bold"> {{$t('买家信息')}} </view>
      <view class="normal sm m-t-20">
        {{$t('买家昵称')}}： {{ orderInfo.user.nickname || "-" }}
      </view>
      <view class="normal sm m-t-20"> {{$t('买家编号')}}： {{ orderInfo.user.sn }} </view>
      <view class="normal sm m-t-20">
        {{$t('买家手机号')}}： {{ orderInfo.user.mobile }}
      </view>
    </view>
    ...
    >
      <view class="black md bold"> {{$t('发票信息')}} </view>
      <view class="normal sm m-t-20">
        {{$t('发票类型')}}： {{ orderInfo.invoice.type == 0 ? $t('普通发票') : $t('专用发票') }}
      </view>
      <view class="normal sm m-t-20">
        {{$t('抬头名称')}}： {{ orderInfo.invoice.name }}
      </view>
      <view class="normal sm m-t-20">
        {{$t('抬头类型')}}： {{ orderInfo.invoice.header_type == 0 ? "个人" : "企业" }}
      </view>
      <view class="normal sm m-t-20" v-if="orderInfo.invoice.duty_number">
        {{$t('企业税号')}}： {{ orderInfo.invoice.duty_number }}
      </view>
      <view class="normal sm m-t-20">
        {{$t('电子邮箱')}}： {{ orderInfo.invoice.email }}
      </view>
      <view class="normal sm m-t-20" v-if="orderInfo.invoice.address">
        {{$t('企业地址')}}： {{ orderInfo.invoice.address }}
      </view>
      <view class="normal sm m-t-20" v-if="orderInfo.invoice.mobile">
        {{$t('企业电话')}}： {{ orderInfo.invoice.mobile }}
      </view>
      <view class="normal sm m-t-20" v-if="orderInfo.invoice.bank">
        {{$t('开户银行')}}： {{ orderInfo.invoice.bank }}
      </view>
      <view class="normal sm m-t-20" v-if="orderInfo.invoice.bank_account">
        {{$t('银行账号')}}： {{ orderInfo.invoice.bank_account }}
      </view>
    </view>

    <view class="bg-white m-t-20" style="padding: 20rpx 20rpx 50rpx 20rpx">
      <view class="black md bold"> {{$t('订单信息')}} </view>
      <view class="normal sm m-t-20">
        {{$t('订单编号')}}： {{ orderInfo.order_sn }}
      </view>
      <view class="normal sm m-t-20">
        {{$t('订单类型')}}： {{ orderInfo.order_type_text }}({{
          orderInfo.order_source_text
        }})
      </view>
      <view class="normal sm m-t-20">
        {{$t('下单时间')}}： {{ orderInfo.create_time }}
      </view>
      <view class="normal sm m-t-20">
        {{$t('支付状态')}}： {{ orderInfo.pay_status_text }}
      </view>
      <view class="normal sm m-t-20">
        {{$t('支付时间')}}： {{ orderInfo.pay_time }}
      </view>
    </view>

    <view class="bg-white m-t-20" style="padding: 20rpx 20rpx 50rpx 20rpx">
      <view class="black md bold"> {{$t('买家留言')}} </view>
      <view class="normal sm m-t-20">
    ...
    <view class="footer bg-white flex row-right">
      ...
      <template v-if="orderInfo.to_ship_btn">
        ...
        >
          <button class="btn solid br60 flex row-center normal">{{$t('去发货')}}</button>
        </router-link>
      </template>
    ...
    <modal height="200rpx" :cancelShow="false" v-model="recycle">
      <view class="black nr" style="height: 200rpx">
        <view class="m-t-40">{{$t('快递公司')}}： {{ logistics.shipping_name }}</view>
        <view class="m-t-20"
          >{{$t('快递公司')}}： {{ logistics.invoice_no }}
...
export default {
  ...
  data() {
    return {
      ...
      itemList: [
        {
          head: this.$t('赏识在于角度的转换'),
          body: $t('只要我们正确择取一个合适的参照物乃至稍降一格去看待他人')+','+$t('值得赏识的东西便会扑面而来'),
          open: true,
          disabled: true,
        },
      ],
